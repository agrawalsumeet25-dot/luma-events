...
.card{background:rgba(15,23,42,.65);border:1px solid rgba(100,116,139,.08);border-radius:16px;overflow:hidden;cursor:pointer;display:flex;flex-direction:column;transition:transform .3s cubic-bezier(.22,1,.36,1),border-color .3s,box-shadow .3s;transform-style:preserve-3d}
.card:hover{transform:translateY(-4px);border-color:rgba(124,58,237,.2);box-shadow:0 16px 48px rgba(0,0,0,.25),0 0 0 1px rgba(124,58,237,.08)}
.cover{height:160px;background-size:cover;background-position:center;background-color:#1e293b;position:relative}
...
function buildCard(e){
  ...
  return `<div class="card" data-id="${esc(e.id)}" tabindex="0"><div class="cover" ${cov}>${activePerson?`<div style="position:absolute;top:10px;left:10px;z-index:2">${ring(sc,'sm')}</div>`:''}<div class="badges"><span class="badge ${s}">${stL(s)}</span></div></div><div class="body"><div class="title">${esc(e.name||'?')}</div><div class="meta"><div class="row">${iconCal()} ${esc(fD(e.start_at))}</div><div class="row">${iconPin()} ${esc(e.venue||e.city||'?')}${e.city&&e.venue?', '+esc(e.city):''}</div></div><div class="cats">${cats}</div><div class="host">${ha?`<img src="${esc(ha)}" alt=""/>`:''}by ${esc(hn)}</div><div class="rsvp-strip"><span class="rsvp-count">${(e.guest_count||0).toLocaleString()} <span>RSVPs</span></span></div></div></div>`;
}
...
function filterAndRender(){
  ...
  for(const[k,lbl]of Object.entries(labels)){
    ...
    h+=`<div class="section-header">${dot}${lbl} <span style="color:#334155;font-weight:400;font-size:13px">(${evts.length})</span></div><div class="grid">${evts.map(buildCard).join('')}</div>`;
  }
  grid.innerHTML=h;
...
S('#search').addEventListener('input',filterAndRender);
